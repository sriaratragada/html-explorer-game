...
export function createBountyBoard(locationId: string, worldTime: number): BountyBoard {
  ...
  for (let i = 0; i < count; i++) {
    ...
    bounties.push(generateBountyQuest(BANDIT_NAMES[nameIdx], locationId, reward));
  }
  return { locationId, bounties, lastRefreshTick: worldTime };
}

export function refreshBountyBoard(board: BountyBoard, worldTime: number): BountyBoard {
  // Refresh every 100 ticks
  if (worldTime - board.lastRefreshTick < 100) return board;
  return createBountyBoard(board.locationId, worldTime);
}
